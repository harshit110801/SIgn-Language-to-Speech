...
import numpy as np
import math
import cv2 #capturing video,allows img processing & computer vision task.Features-obj detection,face recog tracking.
import os, sys
import traceback
import pyttsx3 # python text to speech
from keras.models import load_model # help in implementation of neural network
from cvzone.HandTrackingModule import HandDetector # help in ease of img processing
from string import ascii_uppercase
import enchant  #used to check spelling of words
...
class Application:
    ...
    def __init__(self):
        ...
        self.panel.place(x=100, y=3, width=450, height=500)

        self.panel2 = tk.Label(self.root)  # initialize image panel(skeleton)
        self.panel2.place(x=700, y=90, width=400, height=400)
        ...
        self.panel3.place(x=200, y=530)

        self.T1 = tk.Label(self.root)
        self.T1.place(x=10, y=530)
        self.T1.config(text="Character :", font=("Courier", 10, "bold"))

        self.panel5 = tk.Label(self.root)  # Sentence
        self.panel5.place(x=200, y=550)
        ...
        self.T4.place(x=10, y=570)
        self.T4.config(text="Suggestions :", fg="red", font=("Courier", 10, "bold"))


        self.b1=tk.Button(self.root)
        self.b1.place(x=390,y=570)

        self.b2 = tk.Button(self.root)
        self.b2.place(x=590, y=570)

        self.b3 = tk.Button(self.root)
        self.b3.place(x=790, y=570)

        self.b4 = tk.Button(self.root)
        self.b4.place(x=990, y=570)

        self.speak = tk.Button(self.root)
        self.speak.place(x=1200, y=600)
        self.speak.config(text="Speak", font=("Courier", 10), wraplength=100, command=self.speak_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=1105, y=600)
        self.clear.config(text="Clear", font=("Courier", 10), wraplength=100, command=self.clear_fun)
    ...
    def video_loop(self):
        try:
            ...
            if hands:
                ...
                white = cv2.imread("D:\Harshit Pandey\Sign-Language-To-Text-and-Speech-Conversion-master-20230415T094300Z-001\Sign-Language-To-Text-and-Speech-Conversion-master\\white.jpg")
                ...
                if handz:
                    ...
                    self.panel2.config(image=imgtk)
                     ...
                     #The size of current_symbols can be change by below value
                    self.panel3.config(text=self.current_symbol, font=("Courier", 10))

                    #self.panel4.config(text=self.word, font=("Courier", 30))


                    #The size of suggestion_word can be change by below value
                    self.b1.config(text=self.word1, font=("Courier", 10), wraplength=825, command=self.action1)
                    self.b2.config(text=self.word2, font=("Courier", 10), wraplength=825,  command=self.action2)
                    self.b3.config(text=self.word3, font=("Courier", 10), wraplength=825,  command=self.action3)
                    self.b4.config(text=self.word4, font=("Courier", 10), wraplength=825,  command=self.action4)
                   
                   #The size of sentence can be change by below value
            self.panel5.config(text=self.str, font=("Courier", 10), wraplength=1025)
        except Exception:
    ...
    def speak_fun(self):
        self.speak_engine.say(self.str)#say method on the engine instance for the speech synthesis
        self.speak_engine.runAndWait()#run and wait method, it process the voice commands
